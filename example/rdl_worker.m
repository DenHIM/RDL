function mult_sdae_worker(gd,pls,bls,lw,le,ll,ln,prtrn,sv,fd,dpt,fr,nepo,sl)
...
if prtrn==0
    ...
    my.pretrain = sprintf('rdl%d',prtrn); % pretrain
end
rand('seed',11112);
my.save = sprintf('rdl%d',sv);
my.folder = fd;
...
my.dae.n_epoch = 500; %default 500
my.max_iter = 200;
...
my.ctrgpu = 1;
if my.folder>44 && my.folder<50
    my.graph = 'S-citetr16000.dat';
    my.tgraph = 'S-ncitetr16000.dat';
elseif my.folder>39 && my.folder<45
...
my.fid = fopen(sprintf('%s.log',my.save),'a');
fprintf(my.fid, 'Training RDL\n');
fclose(my.fid);

fprintf(1, 'Training RDL\n');
tic;
S = rdl (S, X, X_valid, X_ori, 0.1,my,perm_idx);
